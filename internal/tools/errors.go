...
var (
	...
	errEvidenceDisabled                  = errors.New("evidence download tools are disabled; set ABNORMAL_ALLOW_EVIDENCE_DOWNLOAD=true")
	errCloudMessageIDRequired            = errors.New("cloud_message_id is required")
	errAttachmentNameRequired            = errors.New("attachment_name is required")
	errSearchAttachmentParamsRequired    = errors.New("message_id, tenant_id, attachment_name, raw_message_id, native_user_id, and recipient_mailbox are required")
	errDetection360InquiryTypeRequired   = errors.New("inquiry_type must be MISSED_ATTACK or FALSE_POSITIVE")
	errDetection360ReportTypeRequired    = errors.New("report_type is required")
	errDetection360ReportTypeInvalid     = errors.New("report_type must be false-positive, false-negative, missed-attack, missed-spam, or missed-graymail")
	errDetection360PortalLinkRequired    = errors.New("portal_link is required for false-positive reports")
	errDetection360MessageFieldsRequired = errors.New("recipient_email, sender_email, and subject are required for this report_type")
)
